...
elegir que barquito queda el de dia o el de noche
...
/*  
 ...
 Lo que hice al principio (que era mas facil hacerlo con el class) fue crear 2 variables array para el x,y de las estrellas, que eran llenadas por dos loop en setup que devolvian y metian al array con push valores random para x,y. En DRAW dibujaba los puntos con los indices del array x,y que tenian los valores random
...
// EN SETUP
   ...
   // esto me da un valor diferente al empezar que se guarda en el array
  // valor en x
  for (let i = 0; i < 100; i++) {
    let i = random(width);
    rx.push(i);
  ...
  //valor en y
  for (let j = 0; j < 100; j++) {
    let j = random(height / 2);
    ry.push(j);
  }
  
   // con esto compruebo que se esten cargando bien los valores a rx o ry
  // for(let k in ry){
  //  print(ry[k]);
  // }
